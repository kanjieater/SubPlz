  ...
  if [ ! -f "${1%.mkv}.ru.json" ] &&
     ...
     ! ([ -f "${1%.mkv}.jp.srt" ] || [ -f "${1%.mkv}.ja.srt" ] ||
      ...
      if [ "$(date +%s -r "$1")" -le "$(($(date +%s) - 300))" ]; then
        echo "stable-ts \"$1\" -o \"${1%.mkv}.ru.json\" --model large-v2 --refine --language ja --fp16=True --denoiser=demucs"
        stable-ts "$1" -o "${1%.mkv}.ru.json" --model large-v2 --refine --language ja --fp16=True  --denoiser=demucs
      fi
  fi
' _ {} \; &&
