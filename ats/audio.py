...
def get_speech_timestamps(audio):
    ...
    return vad_get_speech_timestamps(
        audio, vad_model, 0.25, min_speech_duration_ms=100, min_silence_duration_ms=50
    )  # TODO(YM): play with this idk
...
def log_mel_spectrogram(audio, apply_silence=False, n_mels=80):
    ...
    while current < len(audio) - 3 * HOP_LENGTH:
        ...
        stft = torch.stft(
            chunk, N_FFT, HOP_LENGTH, window=window, return_complex=True, center=False
        )
        ...
        if log_spec.max() > max_mel:
            max_mel = log_spec.max()
