...
class AudioStream:
    ...
    def from_file(cls, path, whole=False):
        try:
            ...
            exit(1)
        title = (
            info.get("format", {}).get("tags", {}).get("title", os.path.basename(path))
        )
...
class TextFile:
    ...
    def text(self, *args, **kwargs):
        return [
            TextParagraph(path=self.path, idx=i, content=o, references=[])
            for i, v in enumerate(Path(self.path).read_text().split("\n"))
            if (o := v.strip()) != ""
        ]
...
class Epub:
    ...
    def from_file(cls, path):
        ...
        spine, toc = (
            list(file.spine),
            [
                file.get_item_with_href(urllib.parse.unquote(x.href.split("#")[0]))
                for x in flatten(file.toc)
            ],
        )
        if None in toc:
            print(
                "Couldn't map toc to chapters, contact the dev, preferably with the epub"
            )
            exit(1)
        ...
        for i, v in enumerate(spine):
            content = BeautifulSoup(
                file.get_item_with_id(v[0]).get_content(), "html.parser"
            )
            # find+string=not_empty doesn't work for some reason??? wtf
            for t in content.find("body").find_all(
                ["p", "li", "blockquote", "h1", "h2", "h3", "h4", "h5", "h6"]
            ):
                if t.get_text().strip():
                    v.append(t.get_text())
            ...
            if len(v) == 2:
                v.append(v[0])
            r.append(
                cls(epub=file, titles=v[2:], idx=i, content=content, is_linear=v[1])
            )
        return r
...
def expand_matches(streams, chapters, ats, sta):
    ...
    for ai, a in enumerate(streams):
        ...
        def add(idx, other=[]):
            ...
            z = [chj] + list(
                takewhile(
                    lambda j: (chi, j) not in sta, range(chj + 1, len(chapters[chi][1]))
                )
            )
...
def print_batches(batches, spacing=2, sep1="=", sep2="-"):
    ...
    for row in rows:
        ...
        print(
            csep.join(
                [r.ljust(width[i] - wcswidth(r) + len(r)) for i, r in enumerate(row)]
            )
        )
    ...
    for si, s in enumerate(subs):
        ...
        if r.strip():
            if False:  # Debug
                r = s["text"] + "\n" + r
            segments.append(
                Segment(text=r, start=s["start"] + offset, end=s["end"] + offset)
            )
        else:
            segments.append(
                Segment(
...
def faster_transcribe(self, audio, **args):
    ...
    return {
        "segments": segments,
        "language": args["language"] if "language" in args else info.language,
    }
...
if __name__ == "__main__":
    ...
    faster_whisper, local_only, quantize = (
        args.pop("faster_whisper"),
        args.pop("local_only"),
        args.pop("quantize"),
    )
    fast_decoder, overlap, batches = (
        args.pop("fast_decoder"),
        args.pop("fast_decoder_overlap"),
        args.pop("fast_decoder_batches"),
    )
