...
def handle_webhook():
    ...
    if request.method == "POST":
        if not request.json:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "Invalid request, expected JSON"}
                ),
                400,
            )
        ...
        print("=" * 50)
        print(
            f"✅ POST received from Bazarr at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        return (
            jsonify({"status": "success", "message": "Payload received and logged"}),
            200,
        )
...
if __name__ == "__main__":
    ...
    print("🌍 Homepage available at http://127.0.0.1:5775/")
    print("👂 Listening for GET/POST at http://127.0.0.1:5775/webhook")
