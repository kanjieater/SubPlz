...
def process_job_file(job_file_path, full_config):
    ...
    try:
        ...
        batch_inputs = BatchParams(subcommand='batch', dirs=[host_target_dir], pipeline=pipeline, config=None)
        ...
        logger.opt(exception=True).error(f"Error processing job {os.path.basename(job_file_path)}: {e}")
        watcher_settings = full_config.get('watcher', {})
        error_dir_path = watcher_settings.get('error_directory')

        if not error_dir_path:
            logger.warning("'error_directory' key not found in config. Failed job file was not moved.")
            return
        if not os.path.isdir(error_dir_path):
            logger.warning(f"The specified error_directory '{error_dir_path}' does not exist. Failed job file was not moved.")
            return
        try:
...
def run_watcher(args):
    ...
    try:
        ...
        if not job_dir or not os.path.isdir(job_dir):
            raise RuntimeError(f"'watcher.jobs' directory not found in config: {job_dir}")
        ...
        if not existing_jobs:
            ...
            logger.info(f"Found {len(existing_jobs)} pre-existing job(s). Processing now...")
        ...
        if poll_interval is not None:
            if not isinstance(poll_interval, (int, float)) or poll_interval <= 0:
                raise ValueError(f"'polling_interval_seconds' must be a positive number, but found: {poll_interval}")
            logger.warning(f"Polling observer enabled with a {poll_interval}-second interval (for WSL/network drives).")
        ...
        logger.opt(exception=True).critical("A fatal error occurred in the watcher's main loop.")
        if 'observer' in locals() and observer.is_alive():
            observer.stop()
            observer.join()
        sys.exit(1)