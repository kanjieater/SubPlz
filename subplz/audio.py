...
def get_audio_idx(all_streams: list, target_lang_code: str, path: str) -> dict | None:
    ...
    if best_match["score"] < 0:  # All tracks were undesirable
        print(
            f"🦈All audio tracks scored negatively. Best undesirable match selected (score: {best_match['score']}) for file: {path}"
        )
    elif (
        ...
        print(
            f"🦈No direct language match for the audio stream '{target_lang_code}' (standardized: '{standardized_target_lang}') for file: {path}"
        )
        ...
        print(
            f"🦈Target language for the audio stream '{target_lang_code}' was not recognized. Selected best available stream based on other heuristics for file: {path}"
        )
