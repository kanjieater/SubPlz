    ...
    with tqdm(source.streams) as bar:
        for batches in bar:
            ...
            if subtitle_path is None and incorrect_subtitle_path is None:
                print(
                    f"❗ Skipping syncing {subtitle_path} since --lang-ext-original and --lang-ext-incorrect were empty"
                )
                continue
            if str(subtitle_path) == str(incorrect_subtitle_path):
                print(
                    f"❗ Skipping syncing {subtitle_path} since the name matches the incorrect timed subtitle"
                )
            if not incorrect_subtitle_path.exists():
                print(
                    f"""❗ Subtitle with incorrect timing not found.: {incorrect_subtitle_path}"""
                )
                continue
            temp_srt_path = ""
            ...
            try:
                ...
                if subtitle_path.suffix != incorrect_subtitle_path.suffix:
                    ...
                    temp_incorrect_srt_path = incorrect_subtitle_path.with_suffix(
                        ".tmp.srt"
                    )

                    print(
                        f"🔄 Converting {subtitle_path} and {incorrect_subtitle_path} to SRT format for Alass"
                    )
                    convert_between_sub_format(
                        str(subtitle_path), str(temp_srt_path), format="srt"
                    )
                    convert_between_sub_format(
                        str(incorrect_subtitle_path),
                        str(temp_incorrect_srt_path),
                        format="srt",
                    )
                    subtitle_path = temp_srt_path
                    incorrect_subtitle_path = temp_incorrect_srt_path
            except Exception as err:
                error_message = f"""❗ Failed to Sanitize subtitles; {subtitle_path}
                    {err}
                    """
                print(error_message)
